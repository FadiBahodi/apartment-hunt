#!/usr/bin/env python3
"""Focused Google Maps fetch for Wave G + Zone H buildings.

For each new building, fetch:
  - CVH Mon 7am (AM shift start — real traffic)
  - CVH Fri 5pm REVERSE (CVH → home, end-of-shift painful direction)
  - King West Mon 7am (downtown reach)

Budget: ~78 + ~25 = 103 buildings × 3 routes = 309 calls ≈ $3
Run with: CONFIRM=1 python3 data/fetch_google_wave_g.py
"""
import json, os, sys, time, urllib.request, urllib.parse, calendar
...
# Load existing google_routes.json
g = json.load(open('data/google_routes.json'))

# Collect buildings to fetch: Wave G + Zone H if exists
targets = []
for fn in ['data/wave4_zones.jsonl', '/tmp/wave_g/zone_high_park.jsonl']:
    ...
    for line in open(fn):
        ...
        try:
            ...
            if d.get('lat') and d.get('lng'):
                targets.append((d['id'], d['lat'], d['lng']))
        except: pass
...
for i, (bid, lat, lng) in enumerate(targets):
    ...
    # CVH Mon 7am (home → work AM)
    if 'mon_7am' not in g['routes'][bid]['cvh']:
        r, status = fetch(lat, lng, CVH[0], CVH[1], MON_7AM)
        if r: g['routes'][bid]['cvh']['mon_7am'] = r; count += 1
        time.sleep(0.05)

    # CVH Fri 5pm REVERSE (work → home PM peak)
    if 'pm_5pm' not in g['routes'][bid]['cvh']:
        r, status = fetch(CVH[0], CVH[1], lat, lng, FRI_5PM)
        if r: g['routes'][bid]['cvh']['pm_5pm'] = r; count += 1
        time.sleep(0.05)

    # King West Mon 7am
    if 'mon_7am' not in g['routes'][bid]['king_west']:
        r, status = fetch(lat, lng, KING_WEST[0], KING_WEST[1], MON_7AM)
...
g['meta']['updated'] = datetime.now().isoformat()
json.dump(g, open('data/google_routes.json', 'w'), indent=2)
print(f'\nDone. +{count} routes. Cost: ~${count*0.01:.2f}')
